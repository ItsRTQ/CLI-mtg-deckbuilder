"""Search-family commands: search, search-tags, suggest, similar, complements.

Bodies split verbatim from the former monolithic ``cli.py``. The former
``explore`` / ``combos`` commands (external EDHREC-style fetches) were REMOVED
in v0.8.0: web research is the agent's job, recorded via ``mtg note``.
"""
